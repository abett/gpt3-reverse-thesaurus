###
### Install and Run Local Development Extention
###

1) clone von github
2) npm install
3) npm run dev
4) In a Cromium Based browser, navigate to extensions, enable devleoper mode and  "load unpacked extension" where you select the "/build" folder, where the compiled version is stored

When you do npm run dev, it will include the localhost:4040 in a new tab, so the frontend dockes also needs to run

Maybe you also have to update hasura:
1)  labbrowser-backend git pull
2) Navigate to labbrowser-backend/graphql-engine/graphql-engine-hasura and there run "migrate apply" and then "hasura metadata apply"
